...
class RoleListingTable(db.Model):
    ...
    Role_Name = db.Column(
        db.String(255), db.ForeignKey('Role_Skill.Role_Name'))
...
class RoleSkillTable(db.Model):
    ...
    Role_Name = db.Column(db.String(255), db.ForeignKey(
        'Role.Role_Name'), primary_key=True)
    Skill_Name = db.Column(db.String(255), db.ForeignKey(
...
# Application
# allow staff to apply for a role and include a description, by adding a row to the Application table 
...
# get the current application status of a staff for a role, by seeing if the row for the staff and role exists in the Application table
...
# get all applications for a role listing
...
def getapplicationsbylistingid(listing_id):
    try:
        ...
        for i in range(len(data)):
            staff_name = db.session.query(StaffTable.Staff_FName, StaffTable.Staff_LName).filter_by(
                Staff_ID=data[i].Staff_ID).first()
            data_dict.append({'application_id': data[i].Application_ID, 'listing_id': data[i].Listing_ID, 'staff_id': data[i].Staff_ID,
                             'staff_fname': staff_name.Staff_FName, 'staff_lname': staff_name.Staff_LName, 'brief_description': data[i].Brief_Description})
...
# allow a staff to delete their application for a role listing
@app.route('/delete_application/<int:listing_id>&<int:staff_id>', methods=['DELETE'])
def delete_application(listing_id, staff_id):
    try:
        data = ApplicationTable.query.filter_by(
            Listing_ID=listing_id, Staff_ID=staff_id).first()
...
# Role_Listing
# gets all Role Listings in RoleListing Table
...
# gets all Role Listings in RoleListing Table that are still open for staff
...
def get_all_role_listings():
    try:
        ...
        # Fetch data from the database using SQLAlchemy
        data = RoleListingTable.query.filter(
            RoleListingTable.Close_Window >= today_date).all()
...
# adds role listing to RoleListing Table
...
# gets Role_Listing by Listing_ID
...
# update Role_Listing by Listing_ID
...
# gets Skills and Proficiency Level by Role Listing ID
...
# gets all Managers (subset of Staff)
...
# Staff_Skill
# gets all Staff Skills for a Staff
...
def get_skill_and_proficiency_for_role(role_name):
    try:
        ...
        skills_and_proficiency_for_role = (
            db.session.query(RoleSkillTable.Skill_Name,
                             RoleSkillTable.Proficiency_Level)
            .filter(RoleSkillTable.Role_Name == role_name)
            .all()
        )
